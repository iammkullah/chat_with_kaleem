    ...
    def _init_collections(self):
        ...
        for doc_type, config in self.doc_config.items():
            ...
            try:
                ...
                # Check the vector size from the collection info
                existing_dim = collection_info.result.config.vectors.size
                if existing_dim != expected_dim:
                    print(f"⚠️ Collection '{collection_name}' has dimension {existing_dim} but expected {expected_dim}. Recreating...")
                    self.qdrant_client.delete_collection(collection_name)
                    recreate = True
            except Exception as e:
                # If get_collection fails, we assume the collection doesn't exist.
                recreate = True
